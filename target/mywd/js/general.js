;(function($){
	...
	$.fn.page = function(lastPage, page, index){
		page  = parseInt(page);
		lastPage = parseInt(lastPage);
		$(".page[data-index="+index+"]").empty();
		if (lastPage != 1) {
			if (lastPage < 7) {
				for (var i = 1; i < lastPage + 1; i++) {
					$(".page[data-index="+index+"]").append('<a class="pageid'+i+'">'+i+'</a>');
				};
				$(".page[data-index="+index+"]").append('<span class="nextpage">下一页<span class="icon-arrow-right2"></span></span>');
			}else{
				if (page + 1 <= 5) {
					for (var i = 1; i < 6; i++) {
						$(".page[data-index="+index+"]").append('<a class="pageid'+i+'">'+i+'</a>');
					};
					$(".page[data-index="+index+"]").append('<span>&hellip;</span><a class="pageid'+lastPage+'">'+lastPage+'</a><span class="nextpage">下一页<span class="icon-arrow-right2"></span></span>');
				}else if (page >= lastPage - 2) {
					$(".page[data-index="+index+"]").append('<a class="pageid1">1</a><span>&hellip;</span>');
					for (var i = lastPage - 4; i < lastPage + 1; i++) {
						$(".page[data-index="+index+"]").append('<a class="pageid'+i+'">'+i+'</a>');
					};
					$(".page[data-index="+index+"]").append('<span class="nextpage">下一页<span class="icon-arrow-right2"></span></span>');
			...
			$(".page[data-index="+index+"] a.pageid"+page+"").addClass("current");
			if (page == lastPage) {
				$(".page[data-index="+index+"] .nextpage").remove();
			}
		}
	};
...
$(document).ready(function(){
	...
	$("body").prepend('<div class="customservice">'
				+'<div class="cs">'
					...
					+'</table>'
					+'<div class="csf">'
						+'<a href="mailto:3311628518@qq.com"><input type="button" value="邮件咨询"></a>'
					+'</div>'
				+'</div>'
				+'<div class="close">'
					+'&times;'
		...
		acctmentcontent = '<ul>'
			...
			+'<li name="nav2">'
				...
				+'<ul>'
					...
					+'<li data-redirect="nav28"><a href="c_cashcoupon.html">现金券</a></li>'
					+'<li data-redirect="nav23"><a href="c_address.html">收货地址</a></li>'
					+'<li data-redirect="nav24"><a href="c_security.html">安全中心</a></li>'
					+'<li data-redirect="nav25"><a href="c_credit.html">积分管理</a></li>'
					+'<li data-redirect="nav26"><a href="c_stock.html">虚拟股份</a></li>'
					+'<li data-redirect="nav27"><a href="c_promotion.html">推广中心</a></li>'
				+'</ul>'
			+'</li>'
		+'</ul>';
		...
		success:function(data){
			...
			if (data.success == 0) {
				...
				var loginstr = '<span>'
					...
					+'<ul class="dd1">'
					+'<li><a href="c_account.html">我的账户</a></li>'
					+'<li id="logout">登出</li>'
					+'</ul>';
	...
	$(".rss input[type=button]").click(function(){
		var emailaddr = $(this).siblings("input[type=text]").val();
		if(!emailaddr){
			alert("请输入邮箱");
